...
export default function ImageBackGroundSplashScreen() {
  // Corrección de llaves {} por corchetes []
  const [imagenIndex, setImagenIndex] = useState(0);
  const [blur, setBlur] = useState(0);
  const [appLista, setAppLista] = useState(false);

  // Arreglo con tus imágenes reales y la palabra require bien escrita
  const imagenes = [
    require('../assets/Charls.jpeg'),
    require('../assets/ChevyPop.jpeg'),
    require('../assets/Pto papa.jpeg'), // Recuerda que el archivo real debe llevar ese espacio
  ];
  ...
  if (!appLista) {
    return (
      <View style={styles.splash}>
        <Image
          source={require('../assets/wave.png')} // Asegúrate de tener wave.png en assets
          style={styles.wave}
        />
